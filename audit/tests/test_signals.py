...
from audit.utils import serialize_model_instance
...
class SignalsTestCase(TestCase):
    ...
    def test_model_data(self):
        ...
        self.assertEqual(model_data['app'], 'audit')
        self.assertEqual(model_data['name'], 'TestModel')
        self.assertEqual(model_data['full_name'], 'audit.tests.test_signals.TestModel')
    ...
    @patch('audit.signals.serialize_model_instance')
    ...
    @patch('audit.signals.extract_process_data')
    @patch('audit.signals.get_process')
    @patch('audit.signals.get_last_access')
    @patch('audit.signals._extract_content_data')
    @patch('audit.tasks.save_model_action')
    @patch('audit.signals.settings')
    ...
    @patch('audit.signals.dynamic_import', return_value=None)
    @patch('audit.signals.logger')
    @patch('audit.signals.pre_save')
    @patch('audit.signals.post_save')
    @patch('audit.signals.pre_delete')
    @patch('audit.signals.settings')
    def test_register_fail(self, settings, pre_delete, post_save, pre_save, logger, dynamic_import):
        settings.LOGGED_MODELS = ('audit.tests.TestClass', )
    ...
    def test_register_ok(self, settings, pre_delete, post_save, pre_save, logger, dynamic_import):
        settings.LOGGED_MODELS = ('audit.fail.FailClass', )
